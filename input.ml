...
(* http://stackoverflow.com/questions/10789601/
  ...
  let rec loop c s =
    if(c = '\r') then
      if(s <> "") then s
        ...
        let ns = if(String.contains valid c && (String.length s) < 16)
...
let place_original_armies gs num =
  let rec loop n lst lgs =
    ...
    let terr = choose_territory gs true in
...
let rec choose_attack gs =
  ...
  | Yes ->
      let rec loop () =
        let afrom = choose_territory gs true in
        ...
        let ato = choose_territory gs false in
        if(false (*TODO: Add adjaceny check*)) then
  ...
  | Repeat ->
      ...
      | None -> let () = draw_message "Cannot repeat previous battle" in
                let () = delay 1.0 in choose_attack gs
      | Some (a,b,c) ->
