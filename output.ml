...
let draw_battle gs attack defend arolls drolls (al, dl)=
  ...
  let () = draw_text aname 200 460 4 acol Center in
  let () = draw_text dname 550 460 4 dcol Center in
  let lose n = ("Loses "^(string_of_int n)^" armies.") in
  let () = draw_text (lose dl) 200 405 2 acol Center in
  let () = draw_text (lose al) 550 405 2 dcol Center in
